...
class conf {
    ...
    private static function getConfigInterface() {
        $interface = static::addGroup("interface", "Interface settings",
    ...
    private static function getConfigSSL() {
        $ssl = static::addGroup("ssl", "SSL", "Protect your site against eavesdropping by " .
    ...
    private static function getConfigURL() {
        $url = static::addGroup("url", "URLs", "Define the URLs that are used to access " .
    ...
    private static function getConfigPath() {
        $path = static::addGroup("path", "Paths", "File and directory locations");
    ...
    private static function getConfigMaps() {
        $maps = static::addGroup("maps", "Mapping support",
    ...
    private static function getConfigImport() {
        $import = static::addGroup("import", "Import", "Importing and uploading photos");
    ...
    private static function getConfigImportCLI(confGroup $import) {
        ...
        $importCliAdd_auto=new confItemBool();
        $importCliAdd_auto->setName("import.cli.add.auto");
        $importCliAdd_auto->setLabel("CLI: Auto add");
        $importCliAdd_auto->setDesc("Add non-existent albums, categories, places and " .
            "people, when a parent is defined.");
        $importCliAdd_auto->setDefault(false);
        $importCliAdd_auto->setInternal();
        $import[]=$importCliAdd_auto;

        $importCliAdd_always=new confItemBool();
        $importCliAdd_always->setName("import.cli.add.always");
        $importCliAdd_always->setLabel("CLI: Auto add always");
        $importCliAdd_always->setDesc("Add non-existent albums, categories, places " .
            "and people, regardsless of whether a parent is defined.");
        $importCliAdd_always->setDefault(false);
        $importCliAdd_always->setInternal();
        $import[]=$importCliAdd_always;
    ...
    private static function getConfigWatermark() {
        $watermark = static::addGroup("watermark", "Watermarking",
    ...
    private static function getConfigRotate() {
        $rotate = static::addGroup("rotate", "Rotation", "Rotate images");
    ...
    private static function getConfigShare() {
        $share = static::addGroup("share", "Sharing", "Sharing photos with non-logged on users");
    ...
    private static function getConfigFeature() {
        $feature = static::addGroup("feature", "Features", "Various features");
    ...
    private static function getConfigDate() {
        $date = static::addGroup("date", "Date and time", "Date and time related settings");
